<div>
    {{-- Loader: shown during the synchronous rescan request --}}
    <div
        wire:loading.flex
        wire:target="rescan"
    ...
    {{-- Form / result / error: hidden during the request --}}
    <div wire:loading.remove wire:target="rescan">

        {{-- FORM STATE --}}
        @if($state === 'form')
            <p style="font-size: 0.875rem; color: #6b7280; margin: 0 0 1.25rem 0;">
                Optionally upload a new version of the RFP to replace the existing primary document. Leave blank to re-analyze the existing file.
            </p>

            <label for="replacementFile" style="display: block; font-size: 0.875rem; font-weight: 600; color: #374151; margin-bottom: 0.5rem;">
                Replace Primary RFP (optional)
            </label>
            <input id="replacementFile" type="file" wire:model="replacementFile"
            ...
            <p style="font-size: 0.75rem; color: #6b7280; margin: 0.5rem 0 0 0;">
                PDF, DOC, DOCX, TXT, CSV, MD (max 20MB). If provided, the existing primary RFP file will be replaced.
            </p>

            <div style="display: flex; align-items: center; gap: 0.625rem; margin-top: 1.5rem;">
                <button type="button" wire:click="rescan" wire:loading.attr="disabled" wire:target="replacementFile, rescan"
                ...
                <span wire:loading wire:target="replacementFile" style="font-size: 0.75rem; color: #6b7280; margin-left: 0.25rem;">
        ...
        @if($state === 'complete')
            ...
            <div style="display: flex; flex-direction: column; align-items: center; padding: 1.5rem 1rem;">
                ...
                <p style="margin: 0.25rem 0 0 0; font-size: 0.875rem; color: #6b7280; text-align: center; max-width: 26rem;">
                    The RFP has been re-analyzed. Open the detail page to review the score, red flags, and requirements.
                </p>
